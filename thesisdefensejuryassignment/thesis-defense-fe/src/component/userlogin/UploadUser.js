...
function UploadUser() {
  ...
  const downloadHandler = (event) => {
    ...
    writeFile(wb, "upload_user_result.xlsx");
  };
    ...
    <HustContainerCard title="Upload Users">
      ...
      >
        ...
        <Grid item xs={8}>
          <LinearProgress
            variant="determinate"
            value={total === 0 ? 0 : (result.length * 100) / total}
          />
        </Grid>
        <Grid item xs={1}>
          <Typography
          ...
          >{`${result.length}/${total}`}</Typography>
        </Grid>
      </Grid>
