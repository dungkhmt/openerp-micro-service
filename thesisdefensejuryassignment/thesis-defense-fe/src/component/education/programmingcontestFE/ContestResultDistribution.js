...
export default function ContestResultDistribution(props) {
  ...
  const handleScale = () => {
    ...
    rankingClone.forEach((item) => {
      ...
      const new_point =
        (old_point - min_point) * (new_range / initial_range) + scaleStart;
      item[1] = new_point;
    });
  ...
  const handleExport = () => {
    ...
    const ws1 = utils.aoa_to_sheet(ranking);
    utils.book_append_sheet(wb, ws1, "Result");

    const ws2 = utils.json_to_sheet([statistics]);
    utils.book_append_sheet(wb, ws2, "Insights");

    // Write the workbook to a file
    writeFile(wb, "ContestResult-" + contestId + ".xlsx");
  };
