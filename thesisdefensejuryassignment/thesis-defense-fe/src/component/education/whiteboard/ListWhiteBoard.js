...
export default function ListWhiteBoard() {
  ...
  const [openDeleteWhiteboardModal, setOpenDeleteWhiteboardModal] =
  ...
  const columns = [
    {
      ...
      field: "id",
      render: (rowData) => (
        <Link to={`${pathname}/whiteboard/${rowData["id"]}?page=1`}>
          {rowData["id"]}
        </Link>
      ),
    },
    ...
    },
    {
      title: "Tổng số trang",
      field: "page",
      render: (rowData) => <p>{rowData["totalPage"]}</p>,
    },
    {
      title: "Người tạo",
      field: "createdUser",
      render: (rowData) => <p>{rowData["createdBy"]}</p>,
    },
    {
      title: "Hành động",
      field: "action",
      render: (rowData) => (
        <Tooltip
          title="Xóa bảng viết"
          aria-label="Xóa bảng viết"
          placement="top"
        >
          <Button
            variant="contained"
            color="secondary"
            onClick={() => handleOpenDeleteWhiteboardModal(rowData["id"])}
          >
      ...
      async () => {
        await request(
          ...
          (res) => {
            ...
            navigate(`${pathname}/whiteboard/${whiteboardId}?page=1`);
          },
          {},
  ...
  return (
    <div>
        ...
        actions={[
          {
            icon: () => (
              <Tooltip
                title="Thêm bảng viết"
                aria-label="Thêm bảng viết"
                placement="top"
              >
                <Button
                  variant="contained"
                  color="primary"
                  onClick={handleOpenSetNameModal}
                >
      ...
      >
        <DialogTitle id="form-dialog-create-whiteboard-name">
          Nhập tên bảng viết
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
            Để tiếp tục, bạn cần nhập tên cho bảng viết.
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
