...
export default function TeacherViewQuizListOfCourse(props) {
  ...
  function toggleQuizStatus(quiz) {
    const successHandler = (res) => {
      successNoti(
        "Thay đổi trạng thái quiz thành công, xem kết quả trên giao diện!",
        true
      );
      quiz.statusId =
        quiz.statusId === "STATUS_PUBLIC" ? "STATUS_PRIVATE" : "STATUS_PUBLIC";
  ...
  const CreateQuizButton = (
    <Button
      color="primary"
      variant="contained"
      onClick={() => navigate(`quiz/create/${courseId}`)}
    >
      <AddIcon /> Thêm mới
    </Button>
  );
  ...
  const tableColumns = [
    {
      ...
      render: (question) => (
        <Link
          to={`/edu/teacher/course/quiz/detail/${question.questionId}/${courseId}`}
        >
          {question.questionId}
        </Link>
      ),
