...
export default function TeacherViewChapterListOfCourse(props) {
  ...
  function toggleChapterStatus(chapter) {
    const successHandler = (res) => {
      successNoti(
        "Thay đổi trạng thái chương thành công, xem kết quả trên giao diện!",
        true
      );
      chapter.statusId =
        chapter.statusId === "STATUS_PUBLIC"
          ? "STATUS_PRIVATE"
          : "STATUS_PUBLIC";
    ...
    request(
      "GET",
      `/edu/class/change-chapter-status/${chapter.chapterId}`,
      successHandler,
      errorHandlers
    );
  }
  ...
  const CreateChapterButton = (
    <Button
      color="primary"
      variant="contained"
      onClick={() => navigate(`chapter/create/${courseId}`)}
    >
      <AddIcon /> Thêm mới
    </Button>
  );
