...
export default function AssignmentListOfClass(props) {
  ...
  function navigateToAssignmentDetailPage(assignmentId) {
    navigate(
      `/edu/${props.userRole}/class/${classId}/assignment/${assignmentId}`
    );
  }

  function navigateToCreateAssignmentPage() {
    navigate(`/edu/teacher/class/${classId}/assignment/create`);
  }
  ...
  return (
    <div className={classes.assignmentsTableContainer}>
      <StandardTable
        ...
        onRowClick={(event, assignment) =>
          navigateToAssignmentDetailPage(assignment.id)
        }
        actions={actions}
      />
    </div>
...
function setAssignmentStatus(assignment) {
  let now = new Date();
  let openTime = new Date(assignment.openTime);
  let closeTime = new Date(assignment.closeTime);
