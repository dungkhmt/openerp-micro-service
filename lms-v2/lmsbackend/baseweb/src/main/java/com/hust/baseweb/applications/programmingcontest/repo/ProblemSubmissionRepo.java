...
public interface ProblemSubmissionRepo extends JpaRepository<ProblemSubmissionEntity, UUID> {
    ...
    @Query(
        "select p.problemSubmissionId, p.timeSubmitted, p.status, p.score, p.runtime, p.memoryUsage, p.sourceCodeLanguages from ProblemSubmissionEntity p where p.userLogin = :user and p.problem = :problem")
    List<Object[]> getListProblemSubmissionByUserAndProblemId(
        @Param("user") UserLogin user, @Param("problem")
