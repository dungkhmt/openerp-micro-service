...
@Slf4j
public class WarehouseServiceImplVD implements WarehouseService {
...
//                .exportPrice(item.getExportPrice())
    ...
    @Override
    public Map<UUID, String> getWarehouseNameMapNotInCache() {
        List<Object[]> results = warehouseRepository.findWarehouseIdAndName();
        Map<UUID, String> map = new HashMap<>();
        for (Object[] result : results) {
            UUID warehouseId = (UUID) result[0];
            String name = (String) result[1];
            map.put(warehouseId, name);
        }
        return map;
    }
    ...
    @Override
    public List<WarehouseDetailWithProduct2> getAllWarehouseDetailHavingProducts(List<String> productIds) {

        Set<UUID> productUuidSet = productIds.stream().map(UUID::fromString).collect(Collectors.toSet());
        Set<Warehouse> warehouses = new HashSet<>();
        for (UUID productId : productUuidSet) {
            List<Warehouse> warehouseList = productWarehouseRepository.findAllWarehouseByProductId(productId);
            warehouses.addAll(warehouseList);
        }

        List<WarehouseDetailWithProduct2> response = new ArrayList<>();
        for (Warehouse wh : warehouses) {
            List<WarehouseDetailWithProduct2.BayWithProduct> listBayInfo = warehouseRepository
                    .findBayWithProductByWarehouseIdAndProductIds(wh.getWarehouseId(), new ArrayList<>(productUuidSet));
            response.add(WarehouseDetailWithProduct2.builder()
                    .id(wh.getWarehouseId().toString())
                    .address(wh.getAddress())
                    .code(wh.getCode())
                    .name(wh.getName())
                    .bayWithProducts(listBayInfo)
                    .build());
        }
        return response;
    }
