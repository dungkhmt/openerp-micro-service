...
import com.hust.wmsbackend.management.repository.repo2.ProductPriceRepository2;
...
public class ProductServiceImplVD implements ProductService {
    ...
    public List<ProductGeneralResponse> getAllProductGeneral() {
        ...
        log.info("danh sach hang hoa sau khi map so luong: " + productOnHandQuantityMap);
    ...
    public BigDecimal getCurrPriceByProductId(UUID productId) {
        ...
        if  (prices.size() == 1) {
            ...
            log.info("id san pham loi:" + productId);
            curPrice = prices.get(0).getPrice();
        }
        return curPrice;
    }
