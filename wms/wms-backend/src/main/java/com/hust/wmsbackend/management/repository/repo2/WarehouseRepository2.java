...
public interface WarehouseRepository2 extends JpaRepository<Warehouse, UUID> {
    ...
    @Query("SELECT new com.hust.wmsbackend.management.model.model2.response.WarehouseDetailWithProduct2$BayWithProduct(b.bayId, b.code, ii.productId, ii.quantityOnHandTotal) " +
            "FROM Warehouse w " +
    ...
    @Query("SELECT w.warehouseId, w.name FROM Warehouse w")
    List<Object[]> findWarehouseIdAndName();
