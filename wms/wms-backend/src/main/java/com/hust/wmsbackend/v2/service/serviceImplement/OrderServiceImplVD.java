...
@Slf4j
public class OrderServiceImplVD implements OrderService {
    ...
    @Override
    public List<OrderGeneralResponse> getAllOrdersByStatuses(String[] orderStatusStr) {
        List<SaleOrderHeader> orders;
        if (orderStatusStr == null) {
            orders = saleOrderHeaderRepository.findAllByOrderByOrderDateDesc();
        } else {
            List<OrderStatus> orderStatuses = Arrays.stream(orderStatusStr)
                    .map(OrderStatus::findByCode)
    ...
    public boolean approve(Principal principal,  String orderIdStr) {
        Optional<SaleOrderHeader> saleOrderHeaderOpt = saleOrderHeaderRepository.findById(UUID.fromString(orderIdStr));
        if (!saleOrderHeaderOpt.isPresent()) {
            log.warn(String.format("Order id %s is not exist", orderIdStr));
            return false;
        }
    ...
    @Override
    @Transactional
    public OrderDetailResponse getOrderDetailForWhManagerById(String orderIdStr) {
        // Thông tin cơ bản
        UUID orderId = UUID.fromString(orderIdStr);
        OrderDetailDTO saleOrderHeader = saleOrderHeaderRepository.findAllBasicInfoOfOrderByOrderId(orderId);
        ...
        return OrderDetailResponse.builder()
                ...
                .createdDate(DateTimeFormat.convertDateToString(DateTimeFormat.DD_MM_YYYY, saleOrderHeader.getCreatedStamp()))
                .paymentMethod(saleOrderHeader.getPaymentType().getName())
                .receiptAddress(saleOrderHeader.getAddressName())
                .totalOrderCost(saleOrderHeader.getTotalOrderCost())
                .status(saleOrderHeader.getStatus().getName())
                .statusCode(saleOrderHeader.getStatus().getCode())
                .cusAddLat(saleOrderHeader.getLatitude())
                .cusAddLon(saleOrderHeader.getLongitude())
                .processedItems(processedItems)
                .remainingItems(remainItems)
                .items(items)
                .build();
