...
const OrderApprovalDetail = (props) => {
    ...
    const [isLoading, setLoading] = useState(true);
    const [historyTableData, setHistoryTableData] = useState([]);
    const [isShowSuccessHistoryModal, setShowSuccessHistoryModal] =
        useState(false);
    const [isShowFailHistoryModal, setShowFailHistoryModal] = useState(false);
    ...
    useEffect(() => {
        async function fetchData() {
            await request(
                "get",
                `${API_PATH_2.ADMIN_SALE_ORDER}/${orderId}`,
                (res) => {
                    var data = res.data;
                    for (var i = 0; i < data?.items?.length; i++) {
                        const cost = data?.items[i]?.priceUnit;
    ...
    const approveOrderButtonHandle = () => {
        request(
            "put",
            `${API_PATH_2.APPROVE_SALE_ORDER}/${orderId}`,
            (res) => {
                if (res.status == 200) {
                    successNoti("Phê duyệt đơn hàng thành công");
                    history.push(`${path.substring(0, path.lastIndexOf("/"))}`);
            ...
            {
                500: () =>
                    errorNoti("Phê duyệt đơn hàng thất bại. Vui lòng thử lại"),
            }
        );
    };
    ...
    const cancelOrderButtonHandle = () => {
        request(
            "put",
            `${API_PATH_2.CANCEL_SALE_ORDER}/${orderId}`,
            (res) => {
                if (res.status == 200) {
                    successNoti("Hủy đơn hàng thành công");
                    history.push(`${path.substring(0, path.lastIndexOf("/"))}`);
    ...
    return isLoading ? (
        ...
        <Fragment>
            <Modal
                open={isShowSuccessHistoryModal}
                onClose={() =>
                    setShowSuccessHistoryModal(!isShowSuccessHistoryModal)
                }
                aria-labelledby="modal-modal-title"
                aria-describedby="modal-modal-description"
            >
            ...
            <Box>
                    ...
                    {orderInfo?.statusCode == "CREATED" && (
                        <Grid className={classes.buttonWrap}>
                            <Button
                                ...
                                type="submit"
                                onClick={approveOrderButtonHandle}
                            >
                                Phê duyệt
                            </Button>
                        </Grid>
                    ...
                    {orderInfo?.statusCode == "CREATED" && (
                        <Grid className={classes.buttonWrap}>
                            <Button
                                ...
                                className={classes.addButton}
                                type="submit"
                                onClick={cancelOrderButtonHandle}
                            >
                                Hủy đơn hàng
                            </Button>
                        </Grid>
                ...
                <Box className={classes.bodyBox}>
                    <Box className={classes.formWrap}>
                        <Grid container spacing={0}>
                            {/* Tai khoan mua, Ngay tao, Phuong thuc thanh toan */}
                            <Grid container spacing={2}>
                                    ...
                                    <Box className={classes.inputWrap}>
                                        <Box className={classes.labelInput}>
                                            <Link
                                                component="button"
                                                onClick={() => {
                                                    setShowSuccessHistoryModal(
                                                        true
                                                    );
                                                }}
                                            >
                                                Số lượt hàng giao thành công
                                            </Link>
                                        </Box>
                                        <TextField
                                            fullWidth
                                            ...
                                            size="small"
                                            value={
                                                orderInfo?.totalSuccessProductCount
                                            }
                                            InputProps={{
                                                readOnly: true,
                                            }}
                                ...
                                <Grid item xs={6}>
                                    <Box className={classes.inputWrap}>
                                        ...
                                        <TextField
                                            ...
                                            size="small"
                                            value={convertToVNDFormat(
                                                orderInfo?.totalSuccessProductCost
                                            )}
                                            InputProps={{
                                                readOnly: true,
                                            }}
                                    ...
                                    <Box className={classes.inputWrap}>
                                        <Box className={classes.labelInput}>
                                            <Link
                                                component="button"
                                                onClick={() => {
                                                    setShowFailHistoryModal(
                                                        true
                                                    );
                                                }}
                                            >
                                                Số lượt hàng giao thất bại
                                            </Link>
                                        </Box>
                                        <TextField
                                            fullWidth
                                            ...
                                            size="small"
                                            value={
                                                orderInfo?.totalFailProductCount
                                            }
                                            InputProps={{
                                                readOnly: true,
                                            }}
                                ...
                                <Grid item xs={6}>
                                    <Box className={classes.inputWrap}>
                                        ...
                                        <TextField
                                            ...
                                            size="small"
                                            value={convertToVNDFormat(
                                                orderInfo?.totalFailProductCost
                                            )}
                                            InputProps={{
                                                readOnly: true,
                                            }}
                ...
                <StandardTable
                    ...
                    data={orderInfo?.items}
                    columns={
                        orderInfo?.statusCode == "COMPLETED"
                            ? [
                                  {
                                      title: "Tên sản phẩm",
                                      field: "productName",
