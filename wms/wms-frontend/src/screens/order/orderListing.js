...
const AdminOrderListing = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      await request(
        "get",
        API_PATH_2.ADMIN_SALE_ORDER + `?orderStatus=${ORDER_STATUS_CODE.DELIVERING_A_PART},${ORDER_STATUS_CODE.APPROVED}`,
        (res) => {
          var data = res.data;
          for (var i = 0; i < data.length; i++) {
            const cost = data[i]?.totalOrderCost;
      ...
      await request(
        "get",
        API_PATH_2.ADMIN_SALE_ORDER + `?orderStatus=${ORDER_STATUS_CODE.CANCELLED},${ORDER_STATUS_CODE.LAST_DELIVERING},${ORDER_STATUS_CODE.COMPLETED},${ORDER_STATUS_CODE.SUCCESS},${ORDER_STATUS_CODE.FAIL},${ORDER_STATUS_CODE.DISTRIBUTED}`,
        (res) => {
          var data = res.data;
          for (var i = 0; i < data.length; i++) {
            const cost = data[i]?.totalOrderCost;
      ...
      <TabContext value={value}>
        ...
        <TabPanel value="3">
          <StandardTable 
            ...
            onRowClick={(event, rowData) => {
              history.push(`${path.replace('/orders', '/delivery-bill')}/${rowData.deliveryBillId}`);
            }}
          />
        </TabPanel>
      </TabContext>
