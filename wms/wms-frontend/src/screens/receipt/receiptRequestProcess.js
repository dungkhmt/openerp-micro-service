...
//                           {
//                             !isDoneReceipt &&
//                             <Tab label="Hàng hóa cần xử lý" value="1" />
//                           }
//                           {
//                             !isDoneReceipt &&
...
const ReceiptRequestProcess = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await request(
        "get",
        API_PATH_2.PROCESS_RECEIPT_REQUEST + "/" + receiptId,
        (res) => {
          setRemainingItems(res.data?.remainingItems);
          setProcessedItems(res.data?.processedItems);
        }
      ...
      await request(
        'get',
        `${API_PATH_2.RECEIPT_BILL_BY_RECEIPT_ID}/${receiptId}`,
        (res) => {
          setReceiptBills(res.data);
        }
      )
  ...
  const saveButtonHandle = () => {
    ...
    request(
      "post",
      API_PATH_2.PROCESS_RECEIPT_REQUEST + `/${receiptInfo.receiptRequestId}?isDone=${isDone}`,
      (res) => {
        if (res.status == 200) {
          successNoti("Lưu xử lý đơn hàng thành công");
          history.push(`${path.substring(0, path.lastIndexOf("/"))}`);
  ...
  const cancelReceiptButtonHandle = () => {
    request(
      "put",
      API_PATH_2.CANCEL_RECEIPT_REQUEST + `/${receiptInfo.receiptRequestId}`,
      (res) => {
        if (res.status == 200) {
          successNoti("Hủy bỏ đơn hàng thành công");
          history.push(`${path.substring(0, path.lastIndexOf("/"))}`);
