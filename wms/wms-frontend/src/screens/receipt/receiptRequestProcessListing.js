...
const ReceiptRequestProcessListing = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await request(
        "get",
        API_PATH_2.SALE_MANAGEMENT_RECEIPT_REQUEST + "?status=APPROVED,IN_PROGRESS",
        (res) => {
          setReceiptTableData(res.data);
        }
      );
      ...
      await request(
        "get",
        API_PATH_2.SALE_MANAGEMENT_RECEIPT_REQUEST + "?status=CANCELLED,COMPLETED",
        (res) => {
          setProcessedReceiptTableData(res.data);
        }
      );
      ...
      <TabContext value={tabValue}>
        ...
        <TabPanel value="1">
          <StandardTable 
            ...
            onRowClick={ (event, rowData) => {
              history.push(`${path}/${rowData.receiptRequestId}`);
            } }
          />
        </TabPanel>
        <TabPanel value="2">
        ...
        <TabPanel value="3">
          <StandardTable 
            ...
            onRowClick={ (event, rowData) => {
              history.push(`${path.replace('process-receipts', 'receipt-bill')}/${rowData.receiptBillId}`);
            } }
          />
        </TabPanel>
      </TabContext>
