...
const ShipmentDetail = ( props ) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await request(
        "get",
        `${API_PATH_2.DELIVERY_MANAGER_SHIPMENT}/${shipmentId}`,
        (res) => {
          setShipmentInfo(res.data);
          setTripTableData(res.data.trips);
        }
      ...
      <TabContext value={tabValue}>
        ...
        <TabPanel value="2">
          <StandardTable 
            ...
            editable={{
              ...
              onRowDelete: selectedIds => new Promise((resolve, reject) => {
                setTimeout(() => {
                  ...
                  console.log('xoa cac id sau: ', availableSelectedIds);
                  if (availableSelectedIds.length > 0) {
                    ...
                    successNoti(`Không có bản ghi hợp lệ để xoá`);
                    resolve();
                  }
                })
              }),
            ...
            onRowClick={(event, rowData) => {
              history.push(`${path.replace('/shipments/:id', '/delivery-trips')}/${rowData.deliveryTripId}`);
            }}
          />
        </TabPanel>
      </TabContext>
