...
export default function TeacherViewTopicListOfCourse(props) {
  ...
  async function getTopicList() {
    ...
    request("GET", `/get-quiz-course-topics-of-course/${courseId}`, successHandler, errorHandlers);
  }
  ...
  const columns = [
    { title: "Topic ID", field: "quizCourseTopicId",
      render: (topic) => (
        <Link to={`/edu/teacher/course/topic/detail/${topic.quizCourseTopicId}/${courseId}`}>
    ...
    {
      title: "Public quizs", field: "numberOfPublishedQuizs",
      cellStyle: { maxWidth: '80px', padding: '5px', textAlign: 'center'}
    },
    {
      title: "Private quizs", field: "numberOfPrivateQuizs",
      cellStyle: { maxWidth: '80px', padding: '5px', textAlign: 'center' }
    },
  ];

 const actions = [{ icon: () => CreateTopicButton, isFreeAction: true }]

  return (
    <Card>
      <CardContent className={classes.tableWrapper}>
        <StandardTable  title="Danh sách chủ đề"
                        columns={columns}
                        data={topicsOfCourse}
                        hideCommandBar
                        options={{
