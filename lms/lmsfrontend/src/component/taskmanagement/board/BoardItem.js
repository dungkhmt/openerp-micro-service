...
const BoardItem = ({ task }) => {
    return (
        <>
            <Box border={"1px solid"} borderRadius={"5px"} borderColor={"#bdbdbd"} px={2} mb={2}>
                <Box display={'flex'} justifyContent={'space-between'} alignItems={'center'}>
                    <CategoryElement
                        categoryId={task.taskCategory.categoryId}
                        value={task.taskCategory.categoryName}
                    />
                    <IconButton aria-label="delete" size="large" onClick={() => alert("click r nha!")}>
                ...
                <Box display={'flex'} alignItems={'center'}>
                    ...
                    <Box>
                        {task.outOfDate && <LocalFireDepartmentIcon color="error" fontSize={'small'}/>}
                    </Box>
                </Box>
            </Box>
        </>
