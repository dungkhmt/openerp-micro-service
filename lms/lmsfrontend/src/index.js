...
export const store = createStore(rootReducer, startState, composeWithDevTools(applyMiddleware(...middleware)))

store.subscribe(() => {
  if (store.getState().auth.token !== undefined) localStorage.setItem('TOKEN', store.getState().auth.token)
})
