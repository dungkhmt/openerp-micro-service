...
//    Page<ContestEntity> getContestByUserAndStatusSuccessful(Pageable pageable, @Param("userLogin") UserLogin userLogin);
    ...
    @Query(
        "select ce from ContestEntity ce where ce.contestId in (select urce.contestId from UserRegistrationContestEntity urce where urce.userId = :userId and urce.status = 'SUCCESSFUL')")
...
//    Page<ContestEntity> getNotRegisteredContestByUserLogin(Pageable pageable, @Param("userLogin") UserLogin userLogin);
    ...
    @Query(
        "select ce from ContestEntity ce where ce.contestId in (select urce.contestId from UserRegistrationContestEntity urce where urce.userId = :userId and urce.status = 'SUCCESSFUL') and ((ce.endTime >= :now and ce.startedAt < :now) or ce.isPublic = true )")
    ...
    Page<ContestEntity> getContestByUserAndStatusSuccessfulInSolvingTime(
        Pageable pageable,
