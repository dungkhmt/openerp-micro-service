        ...
        <DialogContent>
          <div className="flex flex-row gap-4">
            <div className="flex flex-col">
              ...
              <div>Số lượng: {row?.quantity}/{row?.quantityMax}</div>
            ...
            <div className="flex flex-col">
              ...
              <div>Thời gian: {(row?.startTime || row?.endTime) ? `Tiết ${row?.startTime} - Tiết ${row?.endTime}` : `không có`}</div>
              <div>Phòng: {row?.room}</div>
            </div>
          </div>
        </DialogContent>
        <DialogActions>
          <Button disabled={true} onClick={() => {}}>Lưu</Button>
          <Button
            onClick={() => {
              setOpen(false);
            }}
