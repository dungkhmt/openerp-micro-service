...
public interface TaskRepository extends JpaRepository<Task, UUID>, JpaSpecificationExecutor<Task> {
    ...
    @Query("""
            SELECT t.assigneeId AS userId,
                   t.statusId AS statusId,
                   COUNT(t) AS count
            FROM Task t
            WHERE t.assigneeId IN :userIds
            GROUP BY t.assigneeId, t.statusId
        """)
    List<TaskCountProjection> countTasksByUsersAndStatus(@Param("userIds") List<String> userIds);
