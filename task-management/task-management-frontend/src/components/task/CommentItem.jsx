...
const CommentItem = ({
  ...
  return (
    <>
      <Box
        py={2}
        borderBottom={!onBottom ? 1 : 0}
        borderColor={!onBottom ? "#cdb8b8" : "#fff"}
      >
        <Box
          display={"flex"}
          ...
          <Box display={"flex"} alignItems={"center"} mb={2}>
            <Avatar>
              {comment.createdByUserId != null
                ? comment.createdByUserId.charAt(0).toUpperCase()
                : "N"}
            </Avatar>
            <Box px={2} display={"flex"} flexDirection={"column"}>
              ...
              <Box display={"flex"}>
                ...
                <Typography variant="body2">{comment.createdDate}</Typography>
              </Box>
            </Box>
          </Box>
          {comment.modify && (
      ...
      >
        <MenuItem onClick={() => onUpdate(comment.id, comment.comment)}>
          Chỉnh sửa bình luận
        </MenuItem>
