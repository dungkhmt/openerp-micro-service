    ...
    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="vi">
      <Box sx={{mr: 2, bgcolor: 'background.default', minHeight: 'calc(100vh - 64px)'}}>
        <Paper sx={{p: 2, mb: 2}}>
          <Grid container spacing={2} alignItems="center" justifyContent="space-between" wrap="wrap">
            ...
            <Grid item xs={12} md="auto">
              <Button variant="contained" color="primary" startIcon={<AddIcon/>} onClick={() => {
                setNewPayroll({name: "", fromDate: dayjs().subtract(1, 'month').startOf('month'), thruDate: dayjs().subtract(1, 'month').endOf('month')});
          ...
          <Grid container spacing={2} alignItems="center" sx={{mt: 0}}>
            <Grid item xs={12} sm={6} md={5}>
              <TextField fullWidth label="Tìm theo tên kỳ lương" value={searchName}
                         onChange={(e) => setSearchName(e.target.value)} size="small"/>
            </Grid>
        ...
        <Paper sx={{overflow: 'hidden'}}>
          <TableContainer sx={{maxHeight: "calc(100vh - 280px)"}} className="custom-scrollbar">
            <Table stickyHeader size="medium">
              ...
              <TableBody>
                {loading ? (
                  <TableRow><TableCell colSpan={columns.length} align="center" sx={{py: 5}}> <CircularProgress/>
                    <Typography sx={{mt: 1.5}} variant="body1">Đang tải dữ liệu...</Typography> </TableCell></TableRow>
                ) : payrolls.length > 0 ? (
                  payrolls.map((item, index) => (
                    <TableRow key={item.id} hover
                      ...
                      <TableCell sx={{
                        py: 1.2,
                        '&:not(:last-child)': {borderRight: (t) => `1px solid ${t.palette.grey[200]}`}
                      }}>{currentPage * itemsPerPage + index + 1}</TableCell>
                      <TableCell
                        sx={{py: 1.2, '&:not(:last-child)': {borderRight: (t) => `1px solid ${t.palette.grey[200]}`}}}>
                        <Button variant="text" onClick={() => navigate(`${item.id}`)} sx={{
                          textTransform: "none",
                          p: 0,
                      ...
                      <TableCell align="center" sx={{
                        py: 1.2,
                        '&:not(:last-child)': {borderRight: (t) => `1px solid ${t.palette.grey[200]}`}
                      }}>{item.from_date ? dayjs(item.from_date).format("DD/MM/YYYY") : "-"}</TableCell>
                      <TableCell align="center" sx={{
                        py: 1.2,
                        '&:not(:last-child)': {borderRight: (t) => `1px solid ${t.palette.grey[200]}`}
                      }}>{item.thru_date ? dayjs(item.thru_date).format("DD/MM/YYYY") : "-"}</TableCell>
                      <TableCell sx={{
                        py: 1.2,
                        '&:not(:last-child)': {borderRight: (t) => `1px solid ${t.palette.grey[200]}`}
                      }}>{userMap[item.created_by] || item.created_by || "-"}</TableCell>
                      <TableCell align="center" sx={{
                        py: 1.2,
                        '&:not(:last-child)': {borderRight: (t) => `1px solid ${t.palette.grey[200]}`}
                      }}>{renderStatusChip(item.status)}</TableCell>
                  ...
                  <TableRow><TableCell colSpan={columns.length} align="center" sx={{py: 5}}> <Typography variant="h6">Không
                    tìm thấy kỳ lương nào.</Typography> <Typography variant="body1" color="text.secondary"
                                                                    sx={{mt: 1}}> Vui lòng thử lại với từ khóa khác hoặc
                    tạo kỳ lương mới. </Typography></TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </TableContainer>
        ...
        <Dialog open={createModalOpen} onClose={() => setCreateModalOpen(false)} maxWidth="sm" fullWidth
          ...
          <DialogTitle
            sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '1.15rem'}}>
            Tạo kỳ lương mới
            <IconButton aria-label="đóng" size="small" onClick={() => setCreateModalOpen(false)}> <CloseIcon/>
            </IconButton>
          </DialogTitle>
          <DialogContent sx={{pt: '12px'}}>
            <Stack spacing={2.5} sx={{alignItems: 'stretch', paddingTop: '6px'}}>
              <TextField fullWidth label="Tên kỳ lương" value={newPayroll.name}
                         onChange={(e) => setNewPayroll((prev) => ({...prev, name: e.target.value}))} required
                         size="small"/>
              <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="vi">
                <Grid container sx={{width: '100%'}}>
                  ...
                  <Grid item xs={12} sm={6} sx={{
                    pl: {xs: 0, sm: 1},
                    mt: {xs: 2, sm: 0}
                  }}> {/* Thêm padding-left cho item phải (trên màn hình sm trở lên), và margin-top cho xs */}
                    <DatePicker
                      label="Đến ngày"
