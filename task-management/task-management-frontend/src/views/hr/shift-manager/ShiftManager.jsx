...
export default function ShiftManager({ shifts, setShifts }) {
  ...
  const [newMaxEmployees, setNewMaxEmployees] = useState('5'); // Để trống nếu muốn không có giới hạn mặc định
  ...
  const addShift = () => {
    ...
    // Nếu maxEmployees để trống hoặc là '0', coi như không có giới hạn trên (null)
    const maxEmp = newMaxEmployees === '' || newMaxEmployees === '0' ? null : parseInt(newMaxEmployees, 10);

    if (isNaN(minEmp) || minEmp < 0) { setError('NV tối thiểu không hợp lệ (phải là số >= 0).'); return;}
    if (maxEmp !== null && (isNaN(maxEmp) || maxEmp < 0)) { setError('NV tối đa không hợp lệ (phải là số >= 0 hoặc để trống).'); return;}

    // Chỉ kiểm tra max > min khi max có giá trị (không phải null)
    if (maxEmp !== null && minEmp > maxEmp) {
      setError('NV tối thiểu không thể lớn hơn NV tối đa.'); return;
    }
    if (maxEmp !== null && maxEmp > 0 && minEmp > maxEmp ) { // Điều kiện này đã được bao gồm ở trên
      setError('NV tối thiểu không thể lớn hơn NV tối đa.'); return;
    }
    ...
    setNewMinEmployees('1'); setNewMaxEmployees('5'); // Reset về giá trị mặc định
  };
  ...
  const handleShiftChange = (id, field, value) => {
    ...
    if (field === 'minEmployees' || field === 'maxEmployees') {
      if (value === '' || !isNaN(parseInt(value,10))) { // Cho phép chuỗi rỗng
        processedValue = value;
      } else {
        return; // Không cập nhật nếu giá trị nhập vào không phải số và cũng không phải rỗng
      }
    }
  ...
  return (
    <Paper elevation={0} variant="outlined" sx={{ p: { xs: 1.5, sm: 2 }, mb: 2, borderColor: 'rgba(0,0,0,0.08)' }}>
      ...
      <Box component="form" noValidate autoComplete="off" sx={{ mb: 1.5 }}>
        <Grid container spacing={1.5} alignItems="center"> {/* Giữ alignItems center hoặc flex-end */}
          ...
          <Grid item xs={6} sm={3} md={1.5}><TextField fullWidth label="NV Max" type="number" value={newMaxEmployees} onChange={(e) => setNewMaxEmployees(e.target.value)} inputProps={{ min: 0 }} InputLabelProps={{ shrink: true }} placeholder="Để trống/0 = KGH"/></Grid> {/* Bỏ helperText, dùng placeholder */}
          <Grid item xs={12} sm={6} md={1.5} sx={{display: 'flex', alignItems: 'center', height: '40px' /* Căn với TextField */}}>
      ...
      <Box sx={{ maxHeight: '250px', overflowY: 'auto', pr: 0.5 }}> {/* Tăng maxHeight một chút */}
            ...
            <Grid container spacing={1} alignItems="center">
              <Grid item xs={12} sm={12} md={2.5}>
                <TextField
                  ...
                  InputProps={{disableUnderline: false}} // Để lại underline cho cảm giác có thể sửa
                  fullWidth
                  variant="standard"
                  label="Tên ca"
                  value={shift.name}
              ...
              <Grid item xs={6} sm={2.5} md={1.25}>
                <TextField
                  ...
                  label="Min"
                  type="number"
                  value={shift.minEmployees === '' ? '' : shift.minEmployees}
                  onChange={(e) => handleShiftChange(shift.id, 'minEmployees', e.target.value)}
                  inputProps={{ min: 0 }}
                />
              </Grid>
              <Grid item xs={6} sm={2.5} md={1.25}>
                <TextField
                  ...
                  label="Max"
                  type="number"
                  value={shift.maxEmployees === null ? '' : shift.maxEmployees}
                  onChange={(e) => handleShiftChange(shift.id, 'maxEmployees', e.target.value)}
                  inputProps={{ min: 0 }}
                  placeholder="KGH"
                />
              </Grid>
              <Grid item xs={9} sm={2} md={2.5} sx={{display: 'flex', alignItems:'center'}}>
                <FormControlLabel
                  control={<Switch size="small" checked={shift.isNightShift} onChange={(e) => handleShiftChange(shift.id, 'isNightShift', e.target.checked)} />}
                  label="Đêm"
                  sx={{fontSize: '0.8rem'}}
                />
              </Grid>
              <Grid item xs={3} sm={1} md={1} sx={{textAlign: 'right'}}>
                <Tooltip title="Xóa ca">
                  <IconButton onClick={() => removeShift(shift.id)} color="error" size="small">
                    <DeleteIcon />
                  </IconButton>
                </Tooltip>
              </Grid>
