...
const ConfigurableRosterPageInternal = () => {
      ...
      await request(
        ...
        },
        {
          onError: (err) => {
            ...
            toast.error("Lỗi khi tải danh sách bộ cấu hình.");
          }
        }
      );
    } catch (error) {
  ...
  const fetchDepartmentsAPI = useCallback(async () => {
    try {
      ...
      }, {onError: (err) => console.error("Error fetching departments:", err.response?.data || err.message)}, null, {
        params: {
          status: "ACTIVE",
          pageSize: 1000
        }
  ...
  const fetchJobPositionsAPI = useCallback(async () => {
    try {
      ...
      }, {onError: (err) => console.error("Error fetching job positions:", err.response?.data || err.message)}, null, {
        params: {
          status: "ACTIVE",
          pageSize: 1000
        }
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        toast.error("Lỗi tải dữ liệu khởi tạo cho trang.");
      } finally {
  ...
  const handleOpenTemplateModalForNew = () => {
    setEditingTemplate(null);
    setIsTemplateModalOpen(true);
  };
  const handleOpenTemplateModalForEdit = (template) => {
    setEditingTemplate(ensureTemplateStructure(template));
    setIsTemplateModalOpen(true);
  };
  ...
  const handleSaveTemplate = async (templateData) => {
    ...
    try {
      await request(method, endpoint, (res) => {
          toast.success(`Đã ${isUpdating ? 'cập nhật' : 'tạo mới'} bộ cấu hình thành công!`);
        ...
        },
        {
          onError: (err) => {
            ...
            toast.error(`Lỗi khi ${isUpdating ? 'cập nhật' : 'tạo mới'} bộ cấu hình: ${err.response?.data?.message || 'Lỗi không xác định'}`);
            setSnackbarSeverity("error");
          }
        }, payload);
    } catch (error) {
      console.error("Exception during template save:", error);
      toast.error(`Lỗi nghiêm trọng khi ${isUpdating ? 'cập nhật' : 'tạo mới'} bộ cấu hình.`);
    } finally {
  ...
  const handleDeleteTemplateConfirmed = async () => {
    ...
    try {
      await request("delete", `/roster-templates/${templateIdToDelete}`, () => {
          toast.success("Đã xóa bộ cấu hình!");
        ...
        },
        {
          onError: (err) => {
            ...
            toast.error(`Lỗi khi xóa bộ cấu hình: ${err.response?.data?.message || 'Lỗi không xác định'}`);
            setSnackbarSeverity("error");
          }
        }
      );
      ...
      toast.error("Lỗi nghiêm trọng khi xóa bộ cấu hình.");
      setSnackbarSeverity("error");
    } finally {
      setIsSubmitting(false);
      setLoadingTemplatesAction(false);
  ...
  const handleOpenApplyModal = (template) => {
    setTemplateToApply(ensureTemplateStructure(template));
  ...
  const handleActualApplyAndRoster = async (applicationDetails) => {
    ...
    if (originalTemplate) {
      ...
      if (depFiltersChanged || jobFiltersChanged) {
        ...
        try {
          await request("patch", `/roster-templates/${applicationDetails.templateId}`, // Giả sử có endpoint riêng để patch filter
            () => {
            ...
            },
            {
              onError: (err) => {
                ...
                toast.error(`Lỗi khi cập nhật bộ lọc. Tiếp tục xếp lịch với bộ lọc mới.`);
              }
            }, filterPayload);
        } catch (patchError) {
          console.error("Exception during filter patch request:", patchError);
          toast.error(`Lỗi nghiêm trọng khi cập nhật bộ lọc. Tiếp tục xếp lịch với bộ lọc mới.`);
        }
      } else {
        filtersSuccessfullyPatched = true;
      }
    ...
    try {
      await request("post", API_ENDPOINT_GENERATE, (res) => {
          ...
          if (stats) {
            ...
            toast.success(`Xếp lịch thành công! ${createdIds.length} ca mới đã được tạo.`);
          } else {
            const message = `Hoàn tất xử lý. ${createdIds.length > 0 ? `${createdIds.length} ca mới được tạo.` : 'Không có ca nào được tạo.'} (Không có thống kê chi tiết).`;
            if (createdIds > 0) {
          ...
          onError: (err) => {
            ...
            if (err.response) {
              ...
              if (err.response.status === 422 && errorData && errorData.statistics) {
                ...
                setRosterStatistics(errorData.statistics);
                setIsStatsModalOpen(true);
              } else if (errorData && errorData.message) {
                errorMessage = `Lỗi ${err.response.status}: ${errorData.message}`;
      ...
      toast.error("Lỗi cục bộ khi chuẩn bị gửi yêu cầu.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleOpenUndoConfirmModal = () => {
    if (lastGeneratedShiftIds.length === 0) {
      toast.success("Không có ca nào vừa được tạo để hoàn tác.");
      return;
    }
  ...
  const executeUndoLastGeneratedShifts = async () => {
    ...
    try {
      await request("delete", `/shifts`, () => {
        toast.success(`Đã hoàn tác ${lastGeneratedShiftIds.length} ca thành công!`);
        setLastGeneratedShiftIds([]);
      }, {
        onError: (err) => {
          console.error("Lỗi khi hoàn tác ca:", err.response?.data || err.message);
          toast.error("Lỗi khi hoàn tác các ca đã tạo.");
          setSnackbarSeverity("error");
        }
      }, lastGeneratedShiftIds);
    } catch (error) {
      console.error("Lỗi cục bộ khi hoàn tác:", error);
      toast.error("Lỗi cục bộ khi gửi yêu cầu hoàn tác.");
    } finally {
  ...
  return (
    ...
    <>
      <Backdrop sx={{color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1}}
                open={isSubmitting || isUndoingShifts || loadingTemplatesAction}> {/* Tăng zIndex cho Backdrop */}
      ...
      <Box sx={{
          ...
          <Grid container justifyContent="space-between" alignItems="center">
            <Grid item>
              <Typography variant="h4" component="h1"
                          sx={{color: 'text.primary', fontWeight: 600, display: 'flex', alignItems: 'center'}}>
                <ArticleIcon sx={{mr: 1.5, color: 'primary.main', fontSize: '2rem'}}/>
                Quản Lý Bộ Cấu Hình Xếp Lịch
              </Typography>
            </Grid>
            <Grid item>
              <Button color="primary" startIcon={<AddIcon/>} onClick={handleOpenTemplateModalForNew} variant="contained"
                      disabled={isSubmitting || isUndoingShifts || loadingTemplatesAction}>
                Tạo Mới Cấu Hình
              </Button>
            </Grid>
          </Grid>
        ...
        <Box sx={{flexGrow: 1, overflowY: 'auto', p: 2}} className="custom-scrollbar">
          {loadingTemplatesAction && configTemplates.length === 0 ? (
            ...
            }}><CircularProgress/></Box> // Điều chỉnh height
          ) : (
            <TemplateListDisplay
      ...
      {/* Modals */}
      <Modal open={isTemplateModalOpen} onClose={(event, reason) => {
        if (reason !== 'backdropClick' && !isSubmitting) handleCloseTemplateModal();
      }} sx={{display: 'flex', alignItems: 'center', justifyContent: 'center'}} closeAfterTransition>
        <Paper sx={{
          width: '95%',
          maxWidth: '1000px',
          ...
          {isTemplateModalOpen && <TemplateConfigForm onSave={handleSaveTemplate} onCancel={handleCloseTemplateModal}
                                                      initialTemplateData={editingTemplate} isSubmitting={isSubmitting}
                                                      titleStyle={modalTitleStyle}/>}
        </Paper>
      </Modal>

      {templateToApply && (
        <Modal open={isApplyModalOpen} onClose={(event, reason) => {
          if (reason !== 'backdropClick' && !isSubmitting) handleCloseApplyModal();
        }} sx={{display: 'flex', alignItems: 'center', justifyContent: 'center'}} closeAfterTransition>
          <Paper sx={{
            width: '95%',
            maxWidth: '750px',
      ...
      {rosterStatistics && (
        ...
        >
          <DialogTitle sx={{
            ...
            <Box sx={{display: 'flex', alignItems: 'center'}}> <BarChartIcon sx={{mr: 1}}/> Thống Kê Kết Quả Xếp Lịch
            </Box>
          ...
          <DialogContent dividers sx={{
            ...
            <Typography variant="body1" gutterBottom> Cho giai
              đoạn: <strong>{rosterStatistics.rosterStartDate}</strong> đến <strong>{rosterStatistics.rosterEndDate}</strong>
            </Typography>
            <Divider sx={{my: 1.5}}/>
            ...
            <Paper variant="outlined" sx={{p: 1.5, mb: 2, bgcolor: 'background.default'}}>
              <Typography>Tổng số ca đã
                xếp: <strong>{rosterStatistics.totalAssignedShifts ?? 'N/A'}</strong></Typography>
              <Typography>Tổng số giờ làm đã
                xếp: <strong>{rosterStatistics.totalAssignedHours?.toFixed(2) ?? 'N/A'}</strong></Typography>
              {rosterStatistics.fairnessHours && (<> <Typography sx={{mt: 1, fontWeight: '500'}}>Phân Bổ Giờ
                Làm:</Typography> <Typography variant="body2">Giờ làm tối
                thiểu/NV: {rosterStatistics.fairnessHours.minEmployeeValue?.toFixed(2) ?? 'N/A'}</Typography>
                <Typography variant="body2">Giờ làm tối
                  đa/NV: {rosterStatistics.fairnessHours.maxEmployeeValue?.toFixed(2) ?? 'N/A'}</Typography> <Typography
                  variant="body2">Chênh lệch
                  (Max-Min): {rosterStatistics.fairnessHours.rangeValue?.toFixed(2) ?? 'N/A'} giờ</Typography> </>)}
            </Paper>
            <Typography variant="h6" gutterBottom sx={{fontWeight: 500}}>Chi Tiết Theo Nhân Viên</Typography>
            <Paper variant="outlined" className={"custom-scrollbar"} sx={{maxHeight: 250, overflowY: 'auto', mb: 2, bgcolor: 'background.default'}}>
            ...
            {rosterStatistics.detailedRosterLog && rosterStatistics.detailedRosterLog.length > 0 && (
              <> <Typography variant="h6" gutterBottom sx={{fontWeight: 500}}>Log Chi Tiết Quá Trình Xếp
                Lịch</Typography>
                <Paper variant="outlined" className={"custom-scrollbar"} sx={{
                  maxHeight: 300,
                  overflowY: 'auto',
                  p: 1.5,
                  backgroundColor: theme.palette.grey[50]
                }}>
                  {rosterStatistics.detailedRosterLog.map((line, idx) => (
                    <Typography key={idx} component="div" variant="caption" sx={{
                      whiteSpace: 'pre-wrap',
          ...
          <DialogActions sx={{
            ...
            p: '12px 24px',
            backgroundColor: theme.palette.grey[50]
          }}> {/* Nền nhạt cho actions */}
            {lastGeneratedShiftIds.length > 0 && (
              <Button onClick={handleOpenUndoConfirmModal} color="warning" variant="outlined" startIcon={<UndoIcon/>}
                      disabled={isUndoingShifts || isSubmitting}> {isUndoingShifts ? "Đang Hoàn Tác..." : "Hoàn Tác Lịch"} </Button>)}
            <Box sx={{flexGrow: 1}}/>
            <Button onClick={handleCloseStatsModal} color="inherit">Đóng</Button>
            <Button onClick={() => downloadCSV(rosterStatistics)} color="primary" variant="contained"
                    startIcon={<DownloadIcon/>}> Tải CSV </Button>
          </DialogActions>
        </Dialog>
      )}
