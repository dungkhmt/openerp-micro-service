...
public class GAOrderAssignmentStrategy implements DistributeStrategy {
    ...
    private void initializeEmployeeMap(List<Order> orderList){
        ...
        for (Order order : orderList) {
            ...
            Recipient recipient = recipentRepo.findById(order.getRecipientId()).orElseThrow(() -> new NotFoundException("Recipient not found"));
...
if(employees.getFirst() instanceof Collector) {
...
else if(employees.get(0) instanceof Shipper) {
    for (int i = 0; i < size; i++) {
        Recipient recipient1 = recipientMap.get(orderList.get(i).getRecipientId());
        for (int j = 0; j < size; j++) {
            Recipient recipient2 = recipientMap.get(orderList.get(j).getRecipientId());
            if (recipient1.getLatitude() == null || recipient1.getLongitude() == null ||
                    recipient2.getLatitude() == null || recipient2.getLongitude() == null) {
                throw new NotFoundException("Location not found for sender");
            }
            // Tính khoảng cách và lưu vào mảng
            distanceMatrix[i][j] = graphHopperCalculator.calculateRealDistanceFromTo(
                    recipient1.getLatitude(), recipient1.getLongitude(),
                    recipient2.getLatitude(), recipient2.getLongitude()
            );
        }
    }
}
    ...
    public Map<UUID, List<Order>> assignOrderToEmployees(Hub hub, List<Order> orderList, List<Employee> employees) {
        initializeEmployeeMap(orderList);
        initializeDistanceMatrix(orderList, employees); // Khởi tạo mảng khoảng cách
