...
const buildHeaderCells = (columns) => {
    return columns.map((c) => {
        return { id: c.field, label: c.title, numeric: c.type == "numeric", centerHeader: c.centerHeader };
    });
};
...
const EnhancedTableToolbar = (props) => {
    ...
    return (
        ...
        >
            ...
            {isEditable &&
                editable?.onRowAdd != null &&
                editable?.onRowAdd != undefined && (
            ...
            {/*{console.log("Actionsl", actions.length)}*/}
                ...
                actions.map((action) => {
                    ...
                    if (action.iconOnClickHandle === undefined) {
                        ...
                        return (
                            <CommandBarButton
                                onClick={() =>
                                    action.iconOnClickHandle(selectedIds)
                                }
                            >
                                {action.tooltip}
                            </CommandBarButton>
...
const StandardTable = ({
    ...
    const [rowsPerPage, setRowsPerPage] = React.useState(
        options?.pageSize != undefined ? options?.pageSize : 5
    );
    ...
    const isEditable =
        editable != null || editable != undefined || actions != undefined;
    ...
    const handleRowCheckBoxClick = (event, row) => {
        ...
        const selectedIndexData = selectedData?.findIndex(
        ...
        if (selectedIndexData === -1) {
            ...
            newSelectedData.splice(selectedIndexData, 1);
        }
    ...
    const isProcessingItemData = (obj) => {
        if (typeof obj !== 'object' || obj === null) {
            return false;
        }

        // Kiểm tra xem obj có chứa các trường productId, warehouseId, bayId, và quantity không
        if ('productId' in obj && 'warehouseId' in obj && 'bayId' in obj && 'quantity' in obj) {
            return true;
        }
    ...
    const deleteButtonHandle = async () => {
        ...
        if (!!isProcessingItemData(selectedData[0])) {
            await editable.onRowDelete(selectedData);
        } else {
        ...
        const newTableData = rows.filter(
            (row) => !selected.includes(row[rowKey])
        );
    ...
    React.useEffect(() => {
        if (searchQuery == "" || searchQuery == null) {
            ...
            const newRows = originalRows.filter((query) =>
                Object.entries(query).some((entry) =>
                    String(entry[1]).toLowerCase().includes(normQuery)
                )
            );
            setRows(newRows);
        }
            ...
            <Box sx={{ width: "100%" }}>
                <Paper sx={{ width: "100%", mb: 2 }}>
                    <EnhancedTableToolbar
                        ...
                        tableTitle={title}
                        openNewRow={() => setShowAddModal(true)}
                                            ...
                                            >
                                                ...
                                                {headerCells != undefined &&
                                                    headerCells.map((cell) => {
                                                        const columnConfig = columns.find(col => col.field === cell.id); // Tìm config của column
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                {/* Kiểm tra và sử dụng renderCell */}
                                                                {columnConfig?.renderCell ? columnConfig.renderCell(row) : row[cell.id]}
                                                            </TableCell>
                                                        );
                                                    })}
                                                {columns != undefined &&
