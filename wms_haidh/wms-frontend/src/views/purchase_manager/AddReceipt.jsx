...
const AddReceipt = () => {
  ...
  useEffect(() => {
    request("get", "/purchase-manager/receipts/warehouse", (res) => {
  ...
  const fetchProductSuggestions = useCallback(
    debounce((searchTerm) => {
      if (searchTerm) {
        request("get", `/purchase-manager/receipts/product?search=${searchTerm}`, (res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const requestUrl = id ? "/purchase-manager/receipts/update-receipt" : "/purchase-manager/receipts/create-receipt";
    ...
    <Box sx={{ p: 3, display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
        ...
        <Typography variant="h6" gutterBottom sx={{ ml: 1 }}>
          {id ? 'Update Receipt' : 'Add New Receipt'}
        </Typography>
        <Button
          variant="contained"
          color="primary"
