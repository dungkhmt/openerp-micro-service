...
import { columns } from "../../config/price";
...
export default function ProductPriceList() {
    ...
    useEffect(() => {
        request("get", `/products/price?page=${page - 1}&size=${rowsPerPage}&search=${debouncedSearchTerm}`, (res) => {
                ...
                return (
                    <div className="relative flex justify-end items-center gap-2">
                        <Dropdown className="bg-background border-1 border-default-200">
                            ...
                            <DropdownMenu>
                                <DropdownItem onPress={() => handleUpdate(item.productId)}>Configure Price</DropdownItem>
                            </DropdownMenu>
                        </Dropdown>
                    </div>
                );
    ...
    const handleUpdate = (id) => {
        navigate(`/sale-manager/price-config/${id}`);
    };
        ...
        >
            <TableHeader columns={columns}>
                {(column) => (
                    <TableColumn
                        key={column.uid}
                        align={["name"].includes(column.uid) ? "start" : "center"}
                    >
                        {column.name}
                    </TableColumn>
                )}
                ...
                {(item) => (
                    <TableRow key={item.productId}>
                        {(columnKey) => (
                            <TableCell>
                                {columnKey === "price"
                                    ? (item.price === 0 ? "Not configured" : formatPrice(item.price))
