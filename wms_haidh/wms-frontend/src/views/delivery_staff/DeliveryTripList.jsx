...
import { columns, statusOptions } from "../../config/todaydelivery";
...
export default function DeliveryTrip() {
  ...
  useEffect(() => {
    request("get", `/delivery-trips/today?status=${statusFilter}&page=${page - 1}&size=${rowsPerPage}`, (res) => {
  ...
  const handleUpdate = (id) => {
    navigate(`/delivery-staff/delivery-trip/${id}`);
  };
  ...
  return (
    ...
    >
      ...
      <TableBody emptyContent={"Loading ..."} items={items}>
        {(item) => (
          <TableRow key={item.deliveryTripId}>
            {(columnKey) => (
              <TableCell>
