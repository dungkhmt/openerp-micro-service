...
const Putaway = () => {
    ...
    useEffect(() => {
        if (bayId) {
            setLoading(true);
            request('get', `/receipt-items/assigned?bayId=${bayId}&status=${status}&page=${page}&size=${rowsPerPage}`, (res) => {
    ...
    const handleMarkAsPutawayed = (receiptItemId) => {
        request('post', `/receipt-items/${receiptItemId}/mark-as-putawayed`, (res) => {
            if (res.status === 200) {
                request('get', `/receipt-items/assigned?bayId=${bayId}&status=${status}&page=${page}&size=${rowsPerPage}`, (res) => {
                    setItems(res.data.content);
                    setTotalItems(res.data.totalElements);
                                ...
                                : items.length === 0 ? (
                                    ...
                                    items.map((item) => (
                                        <TableRow key={item.receiptItemId}>
                                            ...
                                            <TableCell align='center' width={100}>{item.quantity}</TableCell>
                                            ...
                                            <TableCell align='center' width={100}>
                                                {item.status !== 'PUTAWAYED' && (
                                                    <Tooltip title="Mark as Putawayed">
                                                        <IconButton onClick={() => handleMarkAsPutawayed(item.receiptItemId)} color="success">
                                                            <CheckCircle />
                                                        </IconButton>
                                                    </Tooltip>
                                                )}
