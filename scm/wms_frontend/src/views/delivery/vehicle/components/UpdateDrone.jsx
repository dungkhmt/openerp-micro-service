...
import { useUpdateDrone } from "../../../../controllers/query/delivery-trip-query";
...
const UpdateDrone = ({ setIsAdd, currDrone }) => {
  const methods = useForm({
    mode: "onChange",
    defaultValues: {
      duration: currDrone?.durationTime,
      capacity: currDrone?.capacity,
      speed: currDrone?.speed,
      transportCostPerUnit: currDrone?.transportCostPerUnit,
      userManaged: currDrone?.userManaged?.id,
      waitingCost: currDrone?.waitingCost,
      name: currDrone?.name,
    },
  ...
  const updateDroneQuery = useUpdateDrone({
    id: currDrone?.id,
  });
  ...
  const onSubmit = async (data) => {
    ...
    await updateDroneQuery.mutateAsync(droneParams);
    setIsAdd((pre) => !pre);
    reset();
  };
  return (
