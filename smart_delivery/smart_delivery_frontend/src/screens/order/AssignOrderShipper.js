    ...
    const confirmAssignmentButtonHandle = () => {
        ...
        request(
            ...
            (res) => {
                ...
                }, 8000); // Delay 1 giây để user thấy notification
            },
            {
                500: () => {
                    ...
                    {editingAssignment ? (
                        <Box sx={{ pt: 2 }}>
                            ...
                            <FormControl fullWidth sx={{ mt: 2 }}>
                                <Autocomplete
                                    value={selectedShipperForEdit || null} // Đảm bảo value là null khi không có gì được chọn
                                    onChange={(event, newValue) => { // Sửa: dùng newValue thay vì e.target.value
                                        console.log("Selected shipper:", newValue);
                                        setSelectedShipperForEdit(newValue?.id || null); // Chỉ lưu ID
                                    }}
                                    options={shippers}
                                    ...
                                    isOptionEqualToValue={(option, value) => option?.id === value?.id} // Thêm optional chaining
                                    ...
                                    clearText="Xóa"
                                    noOptionsText="Không tìm thấy shipper"
                                    isClearable
                                />
                            </FormControl>
                        </Box>
