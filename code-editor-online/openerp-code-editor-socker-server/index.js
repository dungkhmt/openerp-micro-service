  ...
  socket.on(
    ...
    ({ fullName, roomId, peerId }) => {
      ...
      socket.on(SOCKET_IO_EVENTS.SEND_CODE_CHANGES, ({ language, source }) => {
        socket.broadcast
          .to(roomId)
          .emit(SOCKET_IO_EVENTS.RECEIVE_CODE_CHANGES, {
            language: language,
            source: source,
          });
      });
      ...
      socket.on(
        SOCKET_IO_EVENTS.UPDATE_USER_PERMISSION,
        ({ userId, accessPermission }) => {
          socket.broadcast
            .to(roomId)
            .emit(SOCKET_IO_EVENTS.REFRESH_USER_PERMISSION, {
              userId,
              accessPermission,
            });
        }
      ...
      socket.on(
        SOCKET_IO_EVENTS.UPDATE_ROOM_PERMISSION,
        ({ roomId, isPublic, accessPermission }) => {
          socket.broadcast
            .to(roomId)
            .emit(SOCKET_IO_EVENTS.REFRESH_ROOM_PERMISSION, {
              isPublic,
              accessPermission,
            });
        }
      ...
      socket.on(SOCKET_IO_EVENTS.DELETE_USER_PERMISSION, ({ userId }) => {
        socket.broadcast
          .to(roomId)
          .emit(SOCKET_IO_EVENTS.REFRESH_DELETE_PERMISSION, { userId });
      });
